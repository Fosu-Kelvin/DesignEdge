...
const Nav = () => {
  ...
  if (hash && !hash.includes("/")) { // ignore routes like #/enroll
    ...
    <div className={`${toggleIcon ? "active01" : ""}`}>
      <header className={`tile_container ${toggleIcon ? "active" : ""}`}>
        ...
        <nav className="non-toggle" >
         <div onClick={handleClassesClick} style={{cursor:"pointer",marginTop:"0.8em"}} >  Classes</div>
      ...
      <ul className={`active03 ${toggleIcon ? "active02" : ""}`}>
        <li style={{marginTop:"2rem"}}>
             ...
             <span onClick={handleClassesClick} style={{cursor:"pointer"}} >  Classes</span>
